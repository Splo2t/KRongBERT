...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, attention_mask, token_type_ids, seg_ids, label):
...
def seq_cls_convert_examples_to_features(args, examples, tokenizer, max_length, task):
    ...
    for i, example in enumerate(examples[:5]):
        ...
        logger.info("seg_ids: {}".format(" ".join([str(x) for x in features[i].seg_ids])))
...
def seq_cls_load_and_cache_examples(args, tokenizer, mode):
    ...
    all_seg_ids = torch.tensor([f.seg_ids for f in features], dtype=torch.long)
    if output_mode == "classification":
        all_labels = torch.tensor([f.label for f in features], dtype=torch.long)
    elif output_mode == "regression":
        all_labels = torch.tensor([f.label for f in features], dtype=torch.float)

    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_seg_ids, all_labels)
    return dataset
