...
class MyWordpieceTokenizer(object):
    def __init__(self, vocab, unk_token, mask_token, max_input_chars_per_word=900):
        self.vocab = vocab
    ...
    def tokenize(self, text):
        ...
        """
        text = ''.join(self.convert(self.tokens(text)))
        output_tokens = []
...
class KrongBertTokenizer(BertTokenizer):
    def setMorph(self,path):
        self.wordpiece_tokenizer = MyWordpieceTokenizer(vocab=self.vocab, unk_token=self.unk_token, mask_token = self.mask_token)
        self.morph = {}
...
if __name__ == '__main__':
    tokenizer= KrongBertTokenizer.from_pretrained('ourbert',strip_accents=False,  lowercase=False)
